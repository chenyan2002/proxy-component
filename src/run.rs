...
use wasmtime::component::{HasSelf, Component, Linker, Resource, ResourceTable, Val};
use wasmtime::*;
use wasmtime_wasi::{WasiCtx, WasiCtxBuilder, WasiView, WasiCtxView, p2::add_to_linker_sync};
...
pub fn run(args: RunArgs) -> anyhow::Result<()> {
    ...
    if let Some(path) = &args.trace {
        bindings::proxy::recorder::replay::add_to_linker::<_, HasSelf<_>>(&mut linker, |logger| logger)?;
