...
pub fn generate(bindings: &Path, output_file: &Path) -> Result<()> {
    let file = std::fs::read_to_string(bindings)?;
    let ast = syn::parse_file(&file)?;
    let traits = find_all_traits(&ast.items, vec![]);
    let mut output = generate_preamble();
    let stubs = generate_stubs(&traits);
...
fn generate_stubs(traits: &[TraitInfo]) -> Vec<Item> {
